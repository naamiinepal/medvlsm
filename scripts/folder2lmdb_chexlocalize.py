...
def folder2lmdb(json_data, img_dir, mask_dir, output_dir, split, write_frequency=1000):
    ...
    for idx, item in enumerate(tbar):
        ...
        mask = raw_reader(
            #osp.join(mask_dir, f"{item['segment_id']}_{item['mask_name']}.png")
            osp.join(mask_dir, f"{item['mask_name']}")
        )
        data = {
            "img": img,
            "mask": mask,
            ...
            "mask_name": item["mask_name"],  # edited
        }
...
def parse_args():
    ...
    parser.add_argument(
        "-j", "--json-dir", type=str, default="", help="the name of json file."
    )
    parser.add_argument(
        "-i", "--img-dir", type=str, default="refcoco+", help="the folder of images."
    )
    parser.add_argument(
        "-m", "--mask-dir", type=str, default="refcoco+", help="the folder of masks."
    )
    parser.add_argument(
        "-o",
        "--output-dir",
