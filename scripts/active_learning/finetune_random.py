        ...
        for dataset, prompts in dataset_prompts.items():
            for p in prompts:
                command = f"python src/train.py \
                    ...
                    experiment_name={model}_al_rs_ft_{train_frac}_{dataset}_{p} \
                    datamodule=img_txt_mask_al/{dataset}_random.yaml \
                    prompt_type={p} \
                    train_frac={train_frac}\
                    model_name={model}\
                    datamodule.batch_size={batch_size} \
                    model.optimizer.lr={lr} \
                    tags='[{model}, {dataset}, finetune_al_rs, {p}]' \
                    output_masks_dir=output_masks/{model}/al_rs/al_rs_{train_frac}/{dataset}/test/{p}\
